// copied and changed from https://github.com/DGriffin91/BevyJam2022-2/blob/main/bevy_fps_controller/src/controller.rs 
...
impl Plugin for PlayerControllerPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.register_type::<PlayerController>()
            .register_type::<PlayerControllerInput>()
            .add_system(player_controller_input)
            .add_system(player_controller_look)
            .add_system(player_controller_move)
            .add_system(player_controller_render);
    }
}
...
pub fn player_controller_look(mut query: Query<(&mut PlayerController, &PlayerControllerInput)>) {
    for (mut controller, input) in query.iter_mut() {
        ...
        //println!("Player pitch is {:?} and yaw is {:?}", controller.pitch, controller.yaw);
    }
}

pub fn player_controller_move(
    time: Res<Time>,
    ...
    //climbable_wall: Query<Entity, &ClimbableWall>,
    ...
    for (entity, input, mut controller, collider, transform, mut velocity) in query.iter_mut() {
        if input.fly {
            controller.move_mode = match controller.move_mode {
                ...
                MoveMode::Ground | MoveMode::WallRun => MoveMode::Noclip,
                //MoveMode::WallRun => MoveMode::Noclip,
            }
        }
        ...
        match controller.move_mode {
            ...
            MoveMode::Ground => {
                if let Some(capsule) = collider.as_capsule() {
                    ...
                    let wall_cast_velocity = Vec3::X * -1.0;
                    //let right_wall_cast_velocity = Vec3::X;
                    ...
                    if let Some(_wall_hit) = wall_hit {
                        // Only enter wall run if you're touching a wall and moving forward
                        if controller.wall_tick >= 1 && keys.pressed(controller.key_forward){
                            println!("You're technically wall running!");
                            accelerate(
                                wish_direction,
                                wish_speed,
                                controller.accel,
                            ...
                            if input.jump {
                                ...
                                start_velocity.x = controller.jump_speed;
                                //start_velocity.z = controller.jump_speed;
                                end_velocity.y = start_velocity.y - controller.gravity * dt;
                                println!("You're technically wall jumping.");
                            }
                            controller.wall_tick = controller.wall_tick.saturating_add(1);
                        }
                    } else {
                        controller.wall_tick = 0
                    };
...
pub fn player_controller_render(
    logical_query: Query<
        (&Transform, &Collider, &PlayerController, &LogicalPlayer),
        With<LogicalPlayer>,
    >,
